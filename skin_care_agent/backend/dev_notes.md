...
## 2026-08-19 — MVP 产品范围收敛与独立定义 — ✅ 已完成

### 本次完成

- 新增 `design/product/skin_care_app_mvp_spec.md`，明确当前 MVP 的核心判断、版本边界、单张照片与单一关注位置模型、核心闭环、信息架构、必要页面、证据门槛、AI 边界、隐私控制、验收条件和开发取舍顺序。
- 将首页收敛为一个皮肤变化主入口；“记录刚刚使用”改为主卡之外的独立快捷入口。
- 将正面、左侧、右侧三视角记录退出 MVP，只在数据模型层保留未来多照片扩展能力。
- 将包装照片识别、OCR 产品建档和图像特征相似历史排序移入后续版本。
- 同步修改 `design/product/skin_care_app_product_spec.md`，使长期产品文档与独立 MVP 文档保持一致，并添加 MVP 文档入口。

### 验证情况

- 检查 MVP 文档全部 585 行和 58 个标题，版本边界、首页层级、证据规则、发布阻断项与现有实现关系均已落位。
- 主产品文档中“做一次完整记录”、三视角首页入口、主卡内并列次级操作等 5 项旧规则残留均为 0。
- 两份文档明确把三视角、包装识别和图像特征相似度排序放在 MVP 之外。
- MVP 文档占位符为 0、长破折号为 0、行尾空白为 0；`git diff --check` 通过。
- 本次只修改产品文档和进度日志，未修改代码，因此无代码测试项。

### 当前阻塞或遗留

- 无文档结构阻塞。
- 新 MVP 文档等待用户审阅；现有 2026-08-12 后端实施计划仍覆盖较大的旧范围，需要在进入代码开发前按新 MVP 边界重新收敛。

### 下一步

- 由用户审阅 `design/product/skin_care_app_mvp_spec.md`，确认必须提供、明确不做和开发取舍顺序。
- 用户确认后，按新 MVP 文档修订后端实施计划与移动端页面流，再进入代码实现。

---

## 2026-08-19 — v0 原型提示词归档 — ✅ 已完成

### 本次完成

- 新增 `design/原型/v0_app_prompts.txt`，按会话原文保存第一次完整原型生成提示词和 MVP 审计后的首页改造提示词。
- 使用纯文本格式保存，便于逐字阅读，也避免新增未明确要求的 Markdown 文档。

### 验证情况

- 归档文件共 776 行；两段提示词标题、开头和最终验收句均各出现 1 次。
- 第一段提示词位于第 8 行，第二段提示词位于第 641 行；行尾空白为 0。
- 本次只新增提示词归档并更新进度日志，无代码测试项。
...
- 后续给 v0 的重要增量提示词继续追加到同一归档文件，并注明用途。

---

## 2026-08-19 — MVP 用户多区域关注模型 — ✅ 已完成

### 本次完成

- 更新 `design/product/skin_care_app_mvp_spec.md`，将一次记录从单一关注位置调整为用户主动选择一个或多个固定面部区域。
- 一张照片可以为多个已选区域分别生成事件时间点，各时间点共用原始照片、拍摄时间和拍摄条件，并按区域独立积累趋势。
- AI 只分析和展示用户确认的区域；未选择区域表示本次没有观察，不能解释为没有变化或状态稳定。
- 后续记录提供上次关注区域的快捷选项，用户确认后可以沿用、增加或移除；系统不追踪单颗痘痘，也不生成跨区域综合结论。
- 同步更新核心闭环、必要页面、照片可比性、AI 边界、降级规则、验收条件、发布阻断项和现有实现关系。

### 验证情况

- MVP 文档共 594 行、58 个标题；单一关注位置等旧规则残留为 0，多区域核心规则命中 16 行。
- 占位符和行尾空白均为 0。
- 检查记录模型、首次与后续流程、页面状态、证据门槛、AI 边界和验收规则，均使用同一套多区域语义。
- 本次只修改产品文档和开发日志，未修改代码，因此无代码测试项。

### 当前阻塞或遗留

- `design/product/skin_care_app_product_spec.md` 仍保留“一个主要关注位置”的旧规则，本次按用户要求未修改，后续需要同步。
- 2026-08-12 后端实施计划和移动端页面流尚未按新的多区域模型收敛。

### 下一步

- 用户确认 MVP 文档后，同步长期产品文档中的关注区域规则。
- 按多区域模型修订后端实施计划与移动端页面流，再进入代码实现。

---

## 2026-08-19 — MVP AI 可量化外观分析边界 — ✅ 已完成

### 本次完成

- 更新 `design/product/skin_care_app_mvp_spec.md`，将 AI 单次照片能力从笼统的中性事实整理扩展为按用户已选区域进行可量化外观观察。
- 明确数量估算的四种结果：未见明确点状变化、约数、数量区间和无法可靠计数；数量较多、连片或照片质量不足时不强行输出精确数字。
- 将空间分布拆为分布形态、覆盖范围和主要位置；分布形态覆盖单处、零散、单片集中、多片集中、广泛散布、连片、混合和无法判断八种状态。
- 固定可见外观词表，包含泛红、凸起、带白色顶点、平坦红色或褐色印记、脱屑、干燥和局部反光，不使用医学皮损分类。
- 明确跨时间照片比较可以覆盖估算数量、覆盖范围、分布形态、泛红范围、凸起与表面状态和平坦色印，不追踪单颗痘痘身份。
- 同步更新核心价值、版本边界、首次记录流程、这次变化卡、证据维度、验收条件、发布阻断项和现有实现关系。

### 验证情况

- MVP 文档共 680 行、64 个标题；八种分布形态全部存在。
- 与估算数量冲突的禁止规则为 0；医学皮损分类、医学严重度、全脸总数和皮肤评分仍保持禁止。
- 占位符和行尾空白均为 0。
- 本次只修改产品文档和开发日志，未修改代码，因此无代码测试项。

### 当前阻塞或遗留

- `design/product/skin_care_app_product_spec.md` 尚未同步多区域模型和可量化外观分析边界。
- 后端实施计划、AI 输出 schema、合规扫描和移动端变化卡尚未按新规则修订。

### 下一步

- 用户确认 MVP 文档后，同步长期产品文档。
- 再按 MVP 规则收敛后端实施计划和移动端页面流，明确 AI 字段、降级值与测试样例。

---

## 2026-08-19 — MVP 区域趋势与时间上下文定稿 — ✅ 已完成

### 本次完成

- 更新 `design/product/skin_care_app_mvp_spec.md`，将趋势收敛为同一区域可见状态随时间的变化，明确切换本次关注区域不会结束、重置或污染其他区域的历史。
- 将一个有效时间点定义为基线、两个有效时间点定义为前后变化、至少三个有效时间点且跨度不少于 7 天定义为阶段趋势。
- 明确事件详情使用按时间排列的区域状态时间线，不以综合皮肤分数作为纵轴，也不强制绘制连续折线；缺失日期保留空档，不做插值或状态推测。
- 将产品使用和生活背景从趋势方向计算中分离，只按真实发生时间作为时间点或观察区间的上下文展示。
- 将跨事件产品反应记忆、关联度计算和“形成个人规律”结论移出 MVP，保留实际使用记录和历史回看。
- 同步调整版本边界、核心模型、核心闭环、信息架构、必要页面、证据门槛、AI 边界、删除重算、验收条件、发布阻断项和实现约束。

### 验证情况

- MVP 文档共 713 行、65 个标题。
- 旧的“两个时间点形成阶段趋势”规则残留为 0；产品反应记忆仅保留在明确不做和后续进入条件中。
- 占位符、长破折号和行尾空白均为 0；关键章节人工复核通过。
- 本次只修改产品文档和开发日志，未修改代码，因此无代码测试项。

### 当前阻塞或遗留

- `design/product/skin_care_app_product_spec.md` 仍保留完整产品的产品反应记忆和旧趋势门槛；它是长期版本设计，本次按用户要求未同步。
- 后端实施计划、数据模型、AI 输出 schema 和移动端历程页面尚未按本次定稿规则修订。

### 下一步

- 以本次 MVP 趋势规则为准，收敛后端实施计划和移动端页面流。
- 实施前明确有效时间点校验、前后变化计算、阶段趋势生成和上下文查询的接口契约与测试样例。

---

## 2026-08-20 — MVP 文档权威链统一 — ✅ 已完成

### 本次完成

- 将 `design/product/skin_care_app_mvp_spec.md` 明确为当前开发与上架版本的唯一产品事实源，覆盖产品范围、流程、数据语义、AI 边界和验收标准。
- 将 `design/product/skin_care_app_product_spec.md` 明确为长期完整产品参考；与当前 MVP 冲突时，以 MVP 文档为准。
- 重写 `project_background.md` 的文档导航和当前状态，移除其定义具体产品规则的职责，并明确早期三视角、皮肤指数、旧趋势和 AI 问答不属于当前 MVP。
- 在 `CLAUDE.md` 中加入产品文档权威规则，要求处理当前 MVP 任务前先读取 MVP 文档，并规定实施计划不得新增或改写产品规则。
- 将 `docs/superpowers/plans/2026-08-12-backend-product-redesign.md` 标记为 `SUPERSEDED` 且禁止继续执行，等待产品歧义全部解决后重新生成实施计划。

### 验证情况

- 已逐项核验 MVP 唯一事实源、长期文档定位、项目背景导航、启动读取规则、冲突优先级和旧计划禁用标记，全部通过。
- 对本次修改的已跟踪文档执行 `git diff --check`，无空白错误；MVP 文档单独检查，未发现行尾空白。
- 本次只修正文档权威关系，没有修改代码，因此无代码测试项。

### 当前阻塞或遗留

- MVP 规格中的区域事件生命周期与记录归属、固定区域可见性、手动状态冲突、趋势生成、删除重算、生活背景、AI 失败恢复和上架基线仍需逐项定稿。
- 新版实施计划尚未生成；在上述产品规则定稿前，旧计划不得恢复执行。

### 下一步

- 明确区域事件的创建、暂停、恢复、结束和重新创建规则，并取消 AI 对事件归属的非确定性判断。

---

## 2026-08-20 — MVP Goal 1 区域事件生命周期与归属定稿 — ✅ 已完成

### 本次完成

- 将区域事件收敛为“当前事件”和“历史事件”两种持久化状态，同一用户的同一区域最多一个当前事件。
- 明确 30 天内自动续接；达到 30 个完整自然日后不自动改变状态，用户再次记录时选择继续原记录或新增一段记录。
- 明确多区域记录只使用一个汇总确认页面，且仅要求达到 30 天门槛的区域确认归属。
- 增加“结束这段观察”：用户主动结束后事件进入历史，同一区域下一次记录直接建立新当前事件；结束不表示医学痊愈。
- 取消 AI 对事件归属的判断，并明确 MVP 不恢复、合并、拆分事件，不迁移保存后的时间点归属。
- 在 design/product/skin_care_app_mvp_spec.md 中同步更新版本边界、核心模型、首次与后续流程、历程状态、缺失语义、AI 边界、核心验收和实现约束。

### 验证情况

- 全文检索确认“暂停追踪”“停止记录”“归属不确定”和 AI 新建或续接事件等旧规则残留为 0。
- 已补充首次建事件、30 天内续接、30 天后两种选择、多区域汇总确认、主动结束及 AI 不可用六类可测试验收场景。
- git diff --check -- skin_care_agent/design/product/skin_care_app_mvp_spec.md 通过；仅有 Git 的 LF/CRLF 行尾转换提示。
- 本次只修改产品规格和开发日志，没有修改代码，因此无代码测试项。

### 当前阻塞或遗留

- Goal 2 至 Goal 8 尚未定稿：固定区域与拍摄顺序、手动状态与有效时间点、趋势决策、AI 异步状态、删除级联、产品使用、生活背景与提醒。
- 30 个完整自然日的时区和发生时间口径将在有效时间点规则中统一确定。

### 下一步

- 进入 Goal 2，定稿固定面部区域字典、区域边界以及区域选择与拍摄的先后顺序。

---

## 2026-08-20 — MVP Goal 2 固定区域与拍照流程定稿 — ✅ 已完成

### 本次完成

- 将面部区域固定为六个：`forehead` 额头、`left_face` 左侧脸、`right_face` 右侧脸、`nose_area` 鼻周、`mouth_area` 口周、`chin` 下巴，并在 MVP 文档中写明互斥边界。
- 明确左右始终以用户本人真实左右为准，自拍镜像只影响预览，不能改变移动端、后端、AI 或历程中的区域 ID。
- 明确每次选择 1 至 6 个区域，不区分主次；跨边界变化可同时选择相邻区域；MVP 不提供其他区域、自由画圈、AI 推荐或自动选区。
- 统一为先拍一张照片、再由用户选择区域；无照片记录使用相同区域字典，AI 只分析用户确认的区域。
- 拍摄页只校验文件是否能够读取，不判断清晰度、光线、姿态、距离或可见性，不显示质量评分或重拍建议。
- 照片证据不足时只让对应 AI 字段返回“无法判断”或“无法可靠计数”，不建立整张照片的质量或可比性状态，也不阻塞原始记录保存。
- 明确区域在保存前可以增删，保存后不能改区、跨区移动或补加区域时间点；选错时删除对应区域时间点或整次记录后重新记录。

### 验证情况

- 已核验六个区域 ID、名称和边界，以及真实左右、镜像不改 ID、1 至 6 区域、无主次、无其他区域、无自由画圈和无 AI 自动选区规则。
- 已核验拍照后选区、无照片共用字典、最多一张照片、仅做文件可读校验、按字段返回无法判断、保存后区域锁定和 AI 仅分析已选区域规则。
- 全文检索未发现旧区域名称或旧的整体照片质量、可比性判断残留。
- `git diff --check` 通过，仅有 Git 的 LF/CRLF 行尾转换提示。
- 本次只修改产品规格和开发日志，没有修改代码，因此无代码测试项。

### 当前阻塞或遗留

- Goal 3 至 Goal 8 尚未定稿：手动状态与有效时间点、趋势决策、AI 异步状态、删除级联、产品使用、生活背景与提醒。
- 现有代码和旧数据中的区域字段尚未迁移；旧的不分左右区域值不能伪装成新的左侧脸或右侧脸证据，具体迁移规则留待新版实施计划处理。

### 下一步

- 进入 Goal 3，定稿手动状态字段、照片与用户输入的关系，以及一条记录何时构成有效区域时间点。

---

## 2026-08-20 — MVP Goal 3 区域状态事实与有效时间点定稿 — ✅ 已完成

### 本次完成

- 将含义模糊的“当前状态”统一为“区域状态事实”，明确用户不需要在每次有照片记录中手动填写。
- 有照片且 AI 对某区域至少输出一项事实时，直接使用 AI 结果；其他字段允许“无法判断”，不向用户展示补充或修改入口。
- 无照片或 AI 对某区域完全没有输出时，用户按区域自由输入；不设固定选项，不判断具体内容，多区域分别填写。
- 用户原文去除首尾空格后非空即可作为状态事实，每个区域最多 500 字；原文永久保留，AI 结构化整理只保存副本且不能覆盖原文。
- 将有效时间点定为已经可靠保存记录发生时间、区域、信息来源，并且至少包含一项 AI 区域事实或一段非空用户原文。
- 明确记录发生时间取点击保存时的当前时间，不支持手动补录过去时间；后端保存 UTC，页面按设备当地时间显示，照片拍摄时间只作元数据。
- 明确事件 30 天门槛按各时间点保存时设备所在地的日历日期计算，不按 720 小时计算。

### 验证情况

- 按用户要求，本次写入后未另行执行文档检索、差异检查或代码测试。
- 本次只修改产品规格和开发日志，没有修改代码。

### 当前阻塞或遗留

- Goal 4 至 Goal 8 尚未定稿：趋势决策、AI 异步状态、删除级联、产品使用、生活背景与提醒。
- 现有后端只保存 `observed_on` 日期，尚未实现本次确定的精确记录发生时间、设备当地日期、区域状态原文和来源字段。

### 下一步

- 进入 Goal 4，定稿照片事实、用户原文和多个时间点证据冲突时的趋势生成规则。

---

## 2026-08-20 — MVP Goal 4 开放式 AI 趋势规则定稿 — ✅ 已完成

### 本次完成

- 明确每个区域时间点只有一种状态输入：照片 AI 事实或用户状态原文，因此 MVP 不建立同时间点“证据冲突”状态。
- 照片 AI 时间点和无照片用户原文时间点进入同一条区域时间线，可以共同支持前后变化和阶段趋势，且每条依据必须保留真实来源。
- 保留确定性门槛：一个有效时间点只作基线，两个时间点只生成前后变化，至少三个时间点且首尾跨度不少于 7 天才允许生成阶段趋势。
- 取消“整体缓和、更明显、基本稳定、变化混合”等固定总体方向枚举，改为由 AI 生成不超过 200 字的具体自然语言摘要。
- 趋势结果同时返回实际引用的时间点、状态事实、信息来源和现有记录无法支持的部分；不同维度方向不同时直接描述具体变化。
- 程序只校验引用归属、来源一致性、长度和安全边界，不用规则算法替代 AI 生成趋势。
- 依据不足或结果校验失败时，固定显示“现有记录还不足以总结阶段变化”。

### 验证情况

- 延续用户要求，本次写入后未另行执行文档检索、差异检查或代码测试。
- 本次只修改产品规格和开发日志，没有修改代码。

### 当前阻塞或遗留

- Goal 5 至 Goal 8 尚未定稿：保存与 AI 异步状态、删除级联、产品使用、生活背景与提醒。
- AI 趋势请求 schema、引用校验器、安全校验和失败恢复机制将在新版实施计划及 Goal 5 中继续定义。

### 下一步

- 进入 Goal 5，定稿原始记录保存、AI 异步处理、失败提示和重试边界。

---

## 2026-08-20 — MVP Goal 5 保存与 AI 异步边界定稿 — ✅ 已完成

### 本次完成

- 明确先可靠保存原始照片或用户原文、区域和记录发生时间，再启动任何 AI 任务；保存成功后立即提示“这次记录已保存”，用户可以离开页面。
- 多区域记录按区域独立显示“AI 整理中”“已完成”或“需要填写”，不建立复杂的整条记录“部分成功”状态，一个区域成功不等待其他区域。
- 单次照片分析成功的区域立即形成有效时间点；最终失败或完全无输出的区域转为用户自由输入，用户不填时只保留原始照片，不进入事件和趋势。
- 前后变化或阶段趋势生成失败不影响已有有效时间点，也不要求用户补填，只显示固定的信息不足说明。
- 保存失败时保留当前页面的照片和区域选择供直接再次提交；支持取消退出，MVP 不做离线队列或跨 App 重启的草稿恢复。
- 明确原始记录创建必须幂等，重复点击和网络重试不能生成重复记录；用户原文提交后，迟到的 AI 结果不能覆盖原文或改变来源。
- 在 MVP 中留下明确 TODO：AI Proxy Gateway 尚待实现，模型调用的超时、重试、退避、熔断和最终失败判定由 Gateway 负责，产品层不实现重试按钮或内部重试策略。
...
- AI Proxy Gateway 是 AI 功能联调和公开测试前必须完成的实施依赖，当前尚未开发。
- Goal 6 至 Goal 8 尚未定稿：删除级联、产品与组合使用、生活背景与提醒。

### 下一步

- 进入 Goal 6，定稿删除照片、区域时间点、整次记录和事件时的级联删除与趋势重算规则。

---

## 2026-08-20 — MVP Goal 6 删除能力移出本期 — ✅ 已完成

### 本次完成

- 根据 MVP 先做增量的取舍，将照片、区域时间点、整次记录、区域事件和账号删除全部移出当前版本。
- 当前 MVP 不提供删除入口或接口，不实现删除级联、删除后趋势重算或数据恢复；记录只做增量追加。
- 保留“结束这段观察”，但明确它只是把当前事件转为历史事件，不删除任何照片、时间点或派生结果。
- 授权撤回只停止后续照片或 AI 能力，不等于删除已经保存的数据。
- 保存后的区域仍不可修改；由于本期没有删除，用户选错后只能新增一条正确记录，原记录继续保留，因此界面必须在保存前确认区域。
- 从版本边界、信息架构、必要页面、验收条件和发布阻断项中移除当前版本的删除要求。
...
- 删除属于后续版本能力；届时需要统一设计共享照片、区域时间点、事件、账号和派生结果的级联规则，不能局部补做。
- Goal 7 和 Goal 8 尚未定稿：产品与组合使用、生活背景与提醒。

### 下一步

- 进入 Goal 7，定稿个人产品、组合和实际使用记录的最小数据与流程。

---

## 2026-08-20 — MVP Goal 7 产品与实际使用记录定稿 — ✅ 已完成

### 本次完成

- 取消独立组合实体、组合命名和组合管理；一次使用选择多个产品时，只保存为同一条实际使用记录。
- 取消平台核心产品库、临时产品审核状态、品牌规格补全和 OCR 建档；个人产品只需用户输入名称即可建立并重复选择。
- 用户不需要先建立或选择产品也能记录实际使用；未选择产品时显示“未注明产品”。
- 实际使用记录仅要求真实使用时间，默认当前时间并允许修改；产品、固定区域和最多 500 字备注均为可选。
- 区域留空时，使用记录作为全局时间背景；填写区域时，只在对应区域历程中展示。
- 同次选择多个产品的使用记录可以出现在各所选产品历史中，但不能拆分为任何单品的疗效或因果证据。
...
- 现有代码尚无正式个人产品和实际使用实体，目前只有 Check-in 日记中的产品名称字符串。
- Goal 8 尚未定稿：生活背景、平稳状态记录和提醒。

### 下一步

- 进入 Goal 8，决定生活背景、平稳状态记录和一次性提醒在当前 MVP 中保留到什么程度。

---

## 2026-08-20 — MVP Goal 8 生活背景贴纸与提醒范围定稿 — ✅ 已完成

### 本次完成

- 保留生活背景，并将其明确为变化卡生成后的可选卡通贴纸环节，不使用普通自由文本或等级表单替代核心趣味体验。
- 每次皮肤变化记录最多保存一份贴纸集合，同次记录的多个区域共用；不提供脱离皮肤变化记录的独立生活背景入口。
- 固定首版六组贴纸：睡眠、压力、饮食、情绪、生理期和护理变化；睡眠、压力、情绪、生理期及护理变化为单选可取消，饮食为多选。
- 为每张贴纸定义稳定 ID 和文字语义；后端只保存 ID，卡通图案和显示文案可以迭代，但不能改变历史 ID 含义。
- 贴纸同时显示图案和文字，只在本次记录及对应区域历程中展示，不交给 AI 改写，不参与有效时间点、事件归属、趋势、相关性或因果分析。
- 将独立“今天没有特别变化”的平稳状态记录移出 MVP。
- 将一次性观察提醒、通知权限和通知调度全部移出 MVP。
- 同步更新版本边界、首页信息架构、必要页面、AI 降级能力、验收条件、实现约束和开发取舍顺序。
...
- 首版贴纸的插画资产和动效尚未设计；语义 ID 已固定，视觉稿可在实现阶段独立完成。
- AI Proxy Gateway 仍是 AI 功能联调和公开测试前的明确实施依赖。
- 八类开发前产品规则已经逐项定稿；新版数据模型、API、迁移、客户端流程和测试实施计划尚未生成。

### 下一步

- 以 `design/product/skin_care_app_mvp_spec.md` 为唯一产品事实源，审计现有 backend 和 mobile 与八类规则的差距，并生成新的分阶段实施计划。

...
> prompt 版本：`full-face-observation-1.2.0` · schema 版本：`full-face-observation-1.0.0`
>
> 来源：`backend/app/services/full_face_prompt.py`
>
> 调用点：`backend/app/services/full_face_analysis_service.py` `analyze_full_face_photo()`
>
> 基线：`main` @ `921eaf9`
...
```json
{
  ...
  "messages": [
    {"role": "system", "content": "<FULL_FACE_OBSERVATION_SYSTEM_PROMPT>"},
    {"role": "user",   "content": "请只整理这张全脸照片能够直接支持的可见外观事实。", "image_urls": ["<原图 data URL>"]}
  ]
}
```
...
```json
{"role": "user", "content": "<FULL_FACE_OBSERVATION_RETRY_PROMPT>"}
```
...
```
上一个结果未通过展示安全校验。请重新观察原图并输出完整七字段 JSON，保留照片能够支持的中性外观事实。只能使用颜色、表面、形态、分布和范围等日常外观词；把任何医学或皮损分类改写为红/粉/褐/肤色、平整/轻微凸起/轻微凹陷、粗糙/颗粒感等可见外观。unknowns 只写光线、角度、分辨率、遮挡、触感或持续时间等照片边界，不得列举医学分类、症状、皮肤类型、产品或建议，即使是否定语境。不要返回空结果，也不要解释校验过程。
```

## Mock 返回

`FULL_FACE_OBSERVATION_MOCK` 供 mock provider 使用：

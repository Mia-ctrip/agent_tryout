...
底部导航固定为观察、历程、产品、我的。照片保存后、AI 结果展示前询问产品使用，AI 同时在后台处理；用户可从产品柜多选、就地搜索目录或自建产品，也可选择今天还没用或暂时跳过。新增产品只加入产品柜，提交使用后才形成使用事实。结果页、当天记录支持继续记录使用；观察首页的区域和产品入口分别跳转历程栏与产品栏。产品栏负责产品柜管理、搜索新增、产品历史及有来源的官方资料，不另设使用记录入口。完整范围、行为与验收只以 MVP 规格为准。
